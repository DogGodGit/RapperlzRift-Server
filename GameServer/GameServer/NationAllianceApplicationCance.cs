...
public class NationAllianceApplicationCancelCommandHandler : InGameCommandHandler<NationAllianceApplicationCancelCommandBody, NationAllianceApplicationCancelResponseBody>
{
	...
	protected override void HandleInGameCommand()
	{
		...
		if (m_nationAllianceApplication.nationId != m_myHero.nationId)
		{
			throw new CommandHandleException(1, "영웅의 국가가 신청한 국가동맹신청이 아닙니다. nationAllianceApplicationId = " + nationAllianceApplicationId);
		}
		...
		m_nationInst.AddFund(m_nationAllianceApplication.fund, m_myHero.id);
		SaveToDB();
		ServerEvent.SendNationAllianceApplicationCanceled(m_nationInst.GetClientPeers(m_myHero.id), nationAllianceApplicationId);
		ServerEvent.SendNationAllianceApplicationCanceled(m_targetNationInst.GetClientPeers(Guid.Empty), nationAllianceApplicationId);
		NationAllianceApplicationCancelResponseBody resBody = new NationAllianceApplicationCancelResponseBody();
		resBody.fund = m_nationInst.fund;
