...
public class BiographyQuestDungeonReviveCommandHandler : InGameCommandHandler<BiographyQuestDungeonReviveCommandBody, BiographyQuestDungeonReviveResponseBody>
{
	...
	{
		if (!(m_myHero.currentPlace is BiographyQuestDungeonInstance currentPlace))
		{
			throw new CommandHandleException(1, "현재 장소에선 사용할 수 없는 명령입니다.");
		}
		if (currentPlace.status != 2)
		{
			throw new CommandHandleException(103, "던전이 부활을 할 수 있는 상태가 아닙니다.");
		}
		if (!m_myHero.isDead)
		{
		...
		if (!currentPlace.biographyQuestDungeon.IsSafeRevivalWaitingTimeElapsed(fElapsedTime))
		{
		...
		BiographyQuestDungeonReviveResponseBody resBody = new BiographyQuestDungeonReviveResponseBody();
