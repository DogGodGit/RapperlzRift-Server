...
public class TaskConsignmentCompleteCommandHandler : InGameCommandHandler<TaskConsignmentCompleteCommandBody, TaskConsignmentCompleteResponseBody>
{
	public const short kResult_NotElapsedCompletionTime = 101;
	...
	protected override void HandleInGameCommand()
	{
		...
		{
			throw new CommandHandleException(101, "아직 완료할 수 있는 시간이 아닙니다.");
		}
		...
		SaveToDB();
		SaveToLogDB();
		TaskConsignmentCompleteResponseBody resBody = new TaskConsignmentCompleteResponseBody();
	...
	private void SaveToDB()
	{
		...
		dbWork.AddSqlCommand(GameDac.CSC_UpdateHeroTaskConsignment_Status(m_heroTaskConsignment.instanceId, 1, m_currentTime));
		dbWork.Schedule();
	}
	...
	private void SaveToLogDB()
	{
		try
		{
			...
			logWork.AddSqlCommand(GameLogDac.CSC_AddHeroTaskConsignmentCompletionLog(logId, m_myHero.id, m_heroTaskConsignment.instanceId, 0, 0L, m_lnRewardExp, m_nGuildFund, m_nGuildBuildingPoint, m_nGuildContributionPoint, m_currentTime));
			foreach (ResultItem resultItem in m_resultItemCollection.resultItems)
			{
