...
public class TradeShipMatchingStartCommandHandler : InGameCommandHandler<TradeShipMatchingStartCommandBody, TradeShipMatchingStartResponseBody>
{
	...
	protected override void HandleInGameCommand()
	{
		...
		TradeShipDifficulty difficulty = tradeShip.GetDifficulty(nDifficulty);
		if (difficulty == null)
		{
		...
		TradeShipSchedule schedule = tradeShip.GetEnterableSchedule(currentTime);
		if (schedule == null)
		{
		...
		if (tradeShip.requiredConditionType == 1)
		{
			int nRequiredHeroLevel = tradeShip.requiredHeroLevel;
		...
		else if (!m_myHero.IsMainQuestCompleted(tradeShip.requiredMainQuestNo))
		{
		...
		if (m_myHero.stamina < tradeShip.requiredStamina)
		{
			throw new CommandHandleException(110, "스태미너가 부족합니다.");
		}
		DateTime currentDate = currentTime.Date;
		if (m_myHero.GetTradeShipAvailableEnterCount(currentDate) <= 0)
		{
			throw new CommandHandleException(111, "입장횟수가 초과되었습니다.");
		}
		if (bIsPartyEntrance)
		{
			Cache.instance.tradeShipMatchingManager.EnterRoom_Party(party, currentTime, new TradeShipMatchingRoomEnterParam(difficulty, schedule));
		}
		else
		{
			Cache.instance.tradeShipMatchingManager.EnterRoom_Single(m_myHero, new TradeShipMatchingRoomEnterParam(difficulty, schedule));
		}
		MatchingRoom matchingRoom = m_myHero.matchingRoom;
		TradeShipMatchingStartResponseBody resBody = new TradeShipMatchingStartResponseBody();
