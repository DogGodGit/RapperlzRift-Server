...
public class ExpDungeonSweepCommandHandler : InGameCommandHandler<ExpDungeonSweepCommandBody, ExpDungeonSweepResponseBody>
{
	...
	protected override void HandleInGameCommand()
	{
		...
		DateValuePair<int> dailyExpDungeonPlayCount = base.hero.dailyExpDungeonPlayCount;
	...
	private void SweepReward()
	{
		...
		m_lnAcquiredExp = (long)Math.Floor((float)lnBaseRewardExp * m_difficulty.expDungeon.sweepExpRewardFactor);
		m_lnAcquiredExp = (long)Math.Floor((float)m_lnAcquiredExp * Cache.instance.GetWorldLevelExpFactor(m_myHero.level));
		VipLevel vipLevel = m_myHero.vipLevel;
		if (vipLevel != null)
		{
			m_lnAcquiredExp = (long)Math.Floor((float)m_lnAcquiredExp * vipLevel.expDungeonAdditionalExpRewardFactor);
		}
