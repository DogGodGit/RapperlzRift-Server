...
public class MainQuestTransformationMonsterSkillCastEventHandler : InGameEventHandler<CEBMainQuestTransformationMonsterSkillCastEventBody>
{
	protected override void HandleInGameEvent()
	{
		...
		if (currentPlace != null)
		{
			...
			if (!m_myHero.isTransformMainQuestMonster)
			{
				throw new EventHandleException("영웅이 메인퀘스트 몬스터변신중이 아닙니다.");
			}
			if (!m_myHero.skillEnabled)
			{
				throw new EventHandleException("영웅이 스킬을 사용할 수 있는 상태가 아닙니다.");
			}
			HeroMainQuestTransformationMonsterSkill mainQuestTransformationMonsterSkill = m_myHero.GetMainQuestTransformationMonsterSkill(nSkillId);
			if (mainQuestTransformationMonsterSkill == null)
			{
				throw new EventHandleException("스킬ID가 유효하지 않습니다. nSkillId = " + nSkillId);
			}
			DateTimeOffset currentTime = DateTimeUtil.currentTime;
			if (!mainQuestTransformationMonsterSkill.IsExpiredSkillCoolTime(currentTime))
			{
				throw new EventHandleException("쿨타임이 만료되지 않았습니다. nSkillId = " + nSkillId);
			}
			MonsterSkill monsterSkill = mainQuestTransformationMonsterSkill.monsterSkill;
			...
			mainQuestTransformationMonsterSkill.castTime = currentTime;
			mainQuestTransformationMonsterSkill.currentHitId = 0;
			m_myHero.StartBattleMode(currentTime);
			ServerEvent.SendHeroMainQuestTransformationMonsterSkillCast(currentPlace.GetDynamicClientPeers(m_myHero.sector, m_myHero.id), m_myHero.id, nSkillId, skillTargetPosition);
			if (monsterSkill.skillType == 2)
			{
				SkillEffect effect = new SkillEffect();
				effect.Init(currentPlace, mainQuestTransformationMonsterSkill.MakeOffense(), skillTargetPosition, Guid.Empty, m_myHero.level);
				currentPlace.AddSkillEffect(effect);
			}
		}
	}
}
public class MainQuestTransformationMonsterSkillHitEventHandler : InGameEventHandler<CEBMainQuestTransformationMonsterSkillHitEventBody>
{
	protected override void HandleInGameEvent()
	{
		Place currentPlace = m_myHero.currentPlace;
		...
		if (mainQuestTransformationMonsterSkill.currentHitId < 0)
		{
		...
		if (nHitId <= mainQuestTransformationMonsterSkill.currentHitId)
		{
			throw new EventHandleException("적중ID가 유효하지 않습니다. nSkillId = " + nSkillId + ", nHitId = " + nHitId + ", mainQuestTransformationMonsterSkillCurrentHitId = " + mainQuestTransformationMonsterSkill.currentHitId);
		}
		mainQuestTransformationMonsterSkill.currentHitId = nHitId;
		Offense offense = mainQuestTransformationMonsterSkill.MakeOffense();
