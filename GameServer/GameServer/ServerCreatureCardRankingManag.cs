...
public class ServerCreatureCardRankingManager
{
	private int m_nRankingNo;

	private List<CreatureCardRanking> m_rankings = new List<CreatureCardRanking>();

	private Dictionary<Guid, CreatureCardRanking> m_rankingsByHero = new Dictionary<Guid, CreatureCardRanking>();

	private SFRunnableStandaloneWork m_updateWork;

	private static ServerCreatureCardRankingManager s_instance = new ServerCreatureCardRankingManager();

	public static ServerCreatureCardRankingManager instance => s_instance;
	...
	private void Task()
	{
		...
		try
		{
			...
			nLastRankingNo = GameDac.LastServerCreatureCardRankingNo(conn, null);
			if (m_nRankingNo == nLastRankingNo)
			{
				return;
			}
			drcRankings = GameDac.ServerCreatureCardRankings(conn, null, nLastRankingNo);
		...
		lock (Global.syncObject)
		{
			...
			{
				CreatureCardRanking ranking = new CreatureCardRanking();
				ranking.Init(dr);
				AddRanking(ranking);
			}
	...
	private void AddRanking(CreatureCardRanking ranking)
	{
		m_rankings.Add(ranking);
		m_rankingsByHero.Add(ranking.heroId, ranking);
	}

	public CreatureCardRanking GetRankingOfHero(Guid heroId)
	{
	...
	public List<PDCreatureCardRanking> GetPDRankings(int nCount)
	{
		List<PDCreatureCardRanking> insts = new List<PDCreatureCardRanking>();
