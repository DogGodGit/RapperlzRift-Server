...
public class HeroProofOfValorBuff
{
	...
	public float physicalDefenseFactor => m_buffBox.physicalDefenseFactor;

	public DateTimeOffset startTime => m_startTime;

	public void Init(Hero target, ProofOfValorBuffBox buffBox, DateTimeOffset startTime)
	{
		if (target == null)
		{
		...
		int nBuffDuration = m_buffBox.proofOfValor.buffDuration * 1000;
		m_bRunning = true;
	...
	private void ProcessTimerTick()
	{
		if (m_bRunning)
		{
			DisposeTimer();
			m_bRunning = false;
			m_target.ProcessProofOfValorBuffFinished();
		}
	}
