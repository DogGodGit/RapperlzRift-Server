...
public class FearAltarMatchingManager : MatchingManager
{
	private FearAltar m_fearAltar;

	public override bool isPartyMatchingEnabled => true;

	public FearAltarMatchingManager(FearAltar fearAltar)
	{
	...
	{
		ServerEvent.SendFearAltarMatchingRoomPartyEnter(peers, (int)matchingStatus, fRemainingTime);
	}
	...
	protected override bool IsEnterableMatchingRoom(Hero hero, DateTimeOffset time, MatchingRoomEntranceParam param)
	{
		...
		if (hero.GetFearAltarAvailableEnterCount(time.Date) <= 0)
		{
			return false;
		}
		if (m_fearAltar.requiredConditionType == 1)
		{
			if (hero.level < m_fearAltar.requiredHeroLevel)
			{
				return false;
			}
		}
		else if (!hero.IsMainQuestCompleted(m_fearAltar.requiredMainQuestNo))
		{
			return false;
		}
		if (hero.stamina < m_fearAltar.requiredStamina)
		{
			return false;
		}
		return true;
	...
	protected override MatchingRoom FindRoom(int nEnterCount, MatchingRoomEntranceParam param)
	{
		...
		foreach (FearAltarMatchingRoom room in m_rooms.Values)
		{
	...
	{
		FearAltarMatchingRoom room = new FearAltarMatchingRoom();
		room.Init(this);
