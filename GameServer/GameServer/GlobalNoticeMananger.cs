...
public class GlobalNoticeMananger
{
	private Dictionary<Guid, GlobalNotice> m_globalNotices = new Dictionary<Guid, GlobalNotice>();
	...
	private static GlobalNoticeMananger s_instance = new GlobalNoticeMananger();

	public object syncObject => m_syncObject;

	public static GlobalNoticeMananger instance => s_instance;

	private GlobalNoticeMananger()
	{
	}
	...
	private void Task(DateTime fromTime, DateTime toTime)
	{
		...
		try
		{
			...
			drcNotices = UserDac.GlobalNotices(conn, null, fromTime, toTime);
		...
		foreach (DataRow dr in drcNotices)
		{
			...
			GlobalNotice notice = new GlobalNotice(id, sContent, nDisplayInterval, nRepetitionCount - 1);
			lock (m_syncObject)
			{
				if (m_bReleased)
				{
				...
				notice.StartTimer();
				m_globalNotices.Add(notice.id, notice);
			}
		}
	}

	public void RemoveGlobalNotice(Guid id)
	{
		m_globalNotices.Remove(id);
	}
	...
	public void Release()
	{
		...
		DisposeUpdateTimer();
		foreach (GlobalNotice notice in m_globalNotices.Values)
		{
			notice.Release();
		}
		m_globalNotices.Clear();
		m_bReleased = true;
	}
}
