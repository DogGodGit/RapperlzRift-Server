...
public class OwnDiaItemUseCommandHandler : InGameCommandHandler<OwnDiaItemUseCommandBody, OwnDiaItemUseResponseBody>
{
	public const short kResult_NotEnoughItem = 101;

	private InventorySlot m_tagetInventorySlot;

	private int m_nResultOwnDia;
	...
	protected override void HandleInGameCommand()
	{
		...
		if (targetItem.type.id != 19)
		{
			throw new CommandHandleException(1, "해당 아이템은 귀속다이아아이템이 아닙니다. nSlotIndex = " + nSlotIndex);
		}
		...
		OwnDiaReward ownDiaReward = Resource.instance.GetOwnDiaReward(targetItem.longValue1);
		if (ownDiaReward != null)
		{
			m_nResultOwnDia = ownDiaReward.value * nUseCount;
		}
		m_myHero.AddOwnDia(m_nResultOwnDia);
		SaveToDB();
		SaveToGameLogDB(targetItem.id, bOwned, nUseCount);
		OwnDiaItemUseResponseBody resBody = new OwnDiaItemUseResponseBody();
